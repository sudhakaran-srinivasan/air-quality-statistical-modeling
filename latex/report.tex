...
\author{Ni Zhao, Pravin Suranthiran, Sudhakaran Srinivasan}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/pm25dist.png}
  \label{fig:pm25dist}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/station_avgs.png}
  \label{fig:station}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/seasonal_avgs.png}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/season_boxplot.png}
  \label{fig:seasonbox}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/monthlytrends.png}
  \label{fig:trend}
...
\begin{figure}[H]
  \includegraphics[width=\linewidth]{figures/corr_map.png}
...
\begin{figure}[H]
  \includegraphics[width=\linewidth]{figures/pm25windspeed.png}
...
The modeling objective was to predict PM2.5 concentration
...
Two prediction tasks were evaluated. The first task was for nowcasting, which
predicted PM2.5 using concurrent pollutant and weather readings, including SO2, NO2, CO,
O3, temperature, pressure, dew point, rainfall, and wind speed. The second task was for
forecasting, which predicted PM2.5 using weather and time features only,
without concurrent pollutant measurements. It is worth noting that as we are using concurrent 
data, this is not a pure forecasting task. Comparing these settings is useful because
...
could dominate the model. Wind direction was also excluded. 
Hour and month were encoded as sine and cosine features to preserve cyclical
time patterns. An 80/20 temporal split was used, meaning the earlier 80\% of observations formed the training set and the final
...
using the total dataset distribution. This may introduce leakage but this was done to ensure 
a consistent metric for evaluation.
Classification models were evaluated using accuracy and
...
\begin{table}[H]
  ...
  \begin{tabular}{llccc}
    ...
    \textbf{Scenario} & \textbf{Model} & \textbf{RMSE} & \textbf{MAE}
                      & $\boldsymbol{R^{2}}$ \\
    \midrule
    Nowcast: pollutants + weather & XGBoost       & 38.742 & 23.322 & 0.780 \\
    Nowcast: pollutants + weather & Random Forest & 38.778 & 23.465 & 0.779 \\
    Nowcast: pollutants + weather & OLS           & 42.079 & 27.956 & 0.741 \\
    Forecast: weather only        & XGBoost       & 57.514 & 39.849 & 0.515 \\
    Forecast: weather only        & Random Forest & 63.258 & 43.499 & 0.414 \\
...
For the all-station regression task, XGBoost was best or comparable. In the
nowcast setting, XGBoost achieved $R^{2} = 0.780$, RMSE $= 38.742$, and MAE $= 23.321$.
In the weather-only forecast setting, performance declined to $R^{2} = 0.515$,
RMSE $= 57.513$, and MAE $= 39.849$. This decline is expected because the forecast
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/regressionmodelsr2.png}
  \label{fig:rsq}
...
\begin{table}[H]
  ...
  \begin{tabular}{llcc}
    ...
    \textbf{Scenario} & \textbf{Classification task} & \textbf{Accuracy}
                      & \textbf{Weighted $F_1$} \\
    \midrule
    Nowcast: pollutants + weather & PM2.5 quintile & 0.617 & 0.613 \\
    Nowcast: pollutants + weather & PM2.5 binary   & 0.869 & 0.869 \\
    Forecast: weather only        & PM2.5 quintile & 0.455 & 0.432 \\
    Forecast: weather only        & PM2.5 binary   & 0.766 & 0.766 \\
    \bottomrule
  \end{tabular}
  \tablenote{Binary classification separates PM2.5 above and below the total dataset median.
...
XGBoost was selected as the final model family because it showed strong and
...
\begin{figure}[H]
  \caption{XGBoost Actual Versus Predicted PM2.5}
  \includegraphics[width=\linewidth]{figures/xgboosthex.png}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/feature_gains.png}
...
The results support XGBoost as the final model family. In the all-station
nowcast regression task, XGBoost achieved $R^{2} = 0.780$, RMSE $= 38.742$, and
MAE $= 23.322$. In the weather-only forecast-style task, XGBoost remained the best model
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3cm}p{9.5cm}}
    ...
    \bottomrule
  \end{tabular}
  \tablenote{Outputs are summarized from the final-modeling notebook.}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \begin{threeparttable}
  \caption{Regression Performance Across Model Scenarios}
  \label{tab:regfull}
  \small
  \begin{tabular}{>{}p{3.0cm}>{}p{2.5cm}>{}p{2.5cm}ccc}
    ...
    \midrule
    Pollutants + weather & All stations & Random Forest & 38.778 & 23.465 & 0.779 \\
    Pollutants + weather & All stations & XGBoost       & 38.742 & 23.322 & 0.780 \\
    Pollutants + weather & All stations & OLS           & 42.079 & 27.956 & 0.741 \\
    Pollutants + weather & Aotizhongxin & XGBoost       & 35.903 & 22.376 & 0.815 \\
    ...
    \bottomrule
  \end{tabular}
  \tablenote{Final-modeling notebook excerpt; feature set and scope combinations used
    an 80/20 temporal train/test split. Other station values included in the notebook.}
  \end{threeparttable}
\end{table}
...
\begin{figure}[H]
  \caption{Quintile Classification Results: Accuracy}
  \includegraphics[width=\linewidth]{figures/class_quint_accuracy.png}
  \label{fig:classacc}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/binary_accuracy.png}
  \label{fig:binacc}
...
\begin{figure}[H]
  ...
  \includegraphics[width=\linewidth]{figures/quintiles.png}
  \label{fig:qcm}
...
The relevant notebook in the repository is
